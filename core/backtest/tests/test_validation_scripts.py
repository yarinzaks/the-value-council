...
class TestEveryScriptIsFound:
    def test_every_agent_plus_the_council(self) -> None:
        # Ten named investors, one factor composite and one quiet
        # compounder. A new agent that forgets its validation script
        # would silently shrink this list instead of failing anything —
        # and an agent added without one never appears on the
        # leaderboard at all.
        assert len(AGENT_SCRIPTS) == 12
        assert len(VALIDATION_SCRIPTS) == 13
