"""Quiet Compounder validation on the **full US market**.
...
per rebalance to ``data/backtest_decisions/quiet_compounder/``.

This agent reads no filings — its whole screen is two measurements on
the price series — so the EDGAR cache is loaded only for the universe
roster, and ``use_fundamentals`` stays off.

Usage::

    .venv/bin/python -m agents.quiet.run_full_market_validation
"""
...
from agents.quiet.strategy import QuietCompounder
...
logger = get_logger("agents.quiet.run_full_market_validation")
...
def main() -> None:
    ...
    print("\nDecisions logged: data/backtest_decisions/quiet_compounder/")
