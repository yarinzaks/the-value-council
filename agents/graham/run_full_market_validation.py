"""Graham Net-Net validation on the **full US market**.

Universe: every SEC active filer with operating XBRL footprint, market
cap >= $500M. Decisions logged per rebalance to
``data/decisions/benjamin_graham/<YYYY-MM-DD>.json``.

Usage::

    .venv/bin/python -m agents.graham.run_full_market_validation
"""
...
logger = get_logger("agents.graham.run_full_market_validation")
...
def main() -> None:
    ...
    cfg = RunnerConfig(
        start_date=date(2019, 12, 30),  # 2020-2024 quick validation
