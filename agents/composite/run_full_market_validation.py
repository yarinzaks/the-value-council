"""Factor Composite validation on the **full US market**.
...
per rebalance to ``data/backtest_decisions/factor_composite/``.

Usage::

    .venv/bin/python -m agents.composite.run_full_market_validation
"""
...
logger = get_logger("agents.composite.run_full_market_validation")
...
def main() -> None:
    ...
    # One loader for both the runner's marks and the momentum leg, so a
    # bar the runner priced a position from is the same bar the score
