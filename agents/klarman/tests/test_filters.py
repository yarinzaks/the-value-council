"""Unit tests for Klarman filters."""
...
class TestDebtToEquity:
    def test_basic(self) -> None:
        f = make_pit("X", total_equity=1_000, total_debt=500)
        assert debt_to_equity(f) == pytest.approx(0.5)
...
class TestPassesQualityGates:
    ...
    def test_high_de_rejected(self) -> None:
        f = make_pit("LEV", total_equity=1_000, total_debt=900)
        result = passes_quality_gates(
            f, market_cap_usd=2_000_000_000.0
        )
        assert not result.passed
